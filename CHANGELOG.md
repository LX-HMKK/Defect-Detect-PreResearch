...
### 新增
- 首页 Hero 改为 WebGL 液态扭曲动态封面，支持鼠标驱动交互。
- 首页算法卡片新增 AirPods Pro 风格横向轮播，深度驱动缩放与模糊过渡。
- 新增四模型对比画廊式网格与得分数字滚动动效。
- 新增 Training Studio 控制中心动效与数字滚动。
- 新增 AirPods Pro 风格单模型推理工作台。
- 新增 `tools/viz` 可视化套件，支持一键生成全部 8 张分析图表（基准热力图、消融敏感性折线、小样本双轴折线、鲁棒性评分卡等）。
- 新增 DRAEM 训练曲线修复与自训练推理崩溃修复后的稳定支持。

### 变更
- 首页改为满屏吸附布局，非 Hero 页放开为自然滚动。
- 全局排版升级：正文基准字号 15px→17px，Hero 标题 96px/600 多行断句，字体升级为 Noto Sans SC Black。
- 按钮几何对齐 Apple CTA 规范（17px/500、11px 21px 内边距）。
- 内容卡移除玻璃模糊，改为纯白卡+细线风格（navbar 等悬浮元素保留玻璃）。
- 推理结果改为左图右信息分栏，移除重复 badge 与旧标题栏。
- 四模型对比共享原图放大为产品主图展示。
- 训练工作室移到第二页，推理表单收窄，页面最大宽度放宽以减少两侧留白。
- 消融折线默认配色统一使用 ALGO_COLORS 常量，并补充 AUROC 端点标注。
- 基准热力图单元格数值改为 2 位小数，PRO 使用 YlOrRd 暖色，注释修正为每行最优单元格。

### 修复
- 修复 DRAEM 训练无曲线及自训练推理崩溃问题。
- 修复首页算法轮播卡片未居中问题。
- 修复 snap 导航滚动兜底同步与比较页页码卡滞。
- 修复第 3 页滚动卡顿与训练工作室滚动卡顿。
- 修复四模型对比热力图 CSS position 泄漏导致容器 0px 不可见的问题。
- 修复 `_common` 损坏文件告警并锁定 6×4 结果契约。
...
### 文档
- `CLAUDE.md` 主文件精简剥离 526→236 行，细则移至参考文件。
- 汇报文档 A 项修复：混淆矩阵、第七章 FastAPI、image_size、措辞与综合排名。
- 汇报文档 B 项补强：早停机制、PaDiM·FRE 取舍、DRAEM 未消融说明、seed。
- 汇报文档 C 项图表替换：基准热力图、消融折线、小样本双轴，删除冗余表。
- 新增 UI 排印+布局对齐 AirPods Pro 设计规范与实施计划。

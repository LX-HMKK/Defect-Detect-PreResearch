...
工业视觉检测长期受限于两个现实：缺陷样本极度稀缺，缺陷形态多样且不可预见。传统有监督方法依赖大量已标注缺陷样本，难以适应高良率产线的实际需求。本课题调研了无监督工业图像异常检测的四大技术路线（重构类、特征建模类、自监督类、预训练大模型类），从中选择三类、四种代表性算法（PatchCore、PaDiM、FRE、DRAEM）进行复现与对比评测。在 MVTec AD 公开数据集和征图 Focusight 企业真实数据集上，于 ≤150 张正常样本的约束下，从图像级检测（AUROC/AUPR）和像素级定位（Pixel AUROC/PRO）四个维度完成了实验验证。
...
3. **企业数据与公开数据之间存在明显的分布偏移**。region2 是所有算法共同的难点（最优仅 PaDiM 73.5% image_AUROC），这是真实工业场景复杂性的直接体现。
4. **PRO 指标是所有方法的共性瓶颈**，最高仅 81.5%（PatchCore @ bottle），企业数据常低于 50%，像素级定位的连续性仍是短板。

本报告包含完整的实验数据、消融分析、混淆矩阵和工程实施建议，可供企业技术选型参考。
...
工业 4.0 和智能制造推高了高端制造对质检的要求。但在成熟的流水线上，检测环节要面对三个相互交织的难题：
...
无监督异常检测的思路正好对应上述矛盾：**不学"什么是缺陷"，只学"什么是正常"**。训练阶段只用正常样本构建正常模式模型，推理时按待测样本偏离正常模式的程度计算异常分数。这种范式有三个优势：
...
无监督工业异常检测的现代研究始于 2019 年 MVTec AD 数据集的发布（Bergmann et al., CVPR 2019）。它提供了统一的评测基准，使领域从方法探索期进入快速发展期。整体演进分四个阶段：

<figure>
  <img src="images/report/evolution_timeline.png" alt="无监督工业异常检测技术演进路线" style="display:block;margin:12px auto;max-width:100%;width:900px;">
  <figcaption align="center">图 1 无监督工业异常检测技术演进路线</figcaption>
</figure>
...
<figure>
  <img src="images/report/method_taxonomy.png" alt="无监督工业异常检测方法分类" style="display:block;margin:12px auto;max-width:100%;width:900px;">
  <figcaption align="center">图 2 无监督工业异常检测方法分类</figcaption>
</figure>
...
3. **工业应用的价值存疑**：据现有文献，WinCLIP 等零样本方法在 MVTec AD 上的细粒度定位精度（Pixel AUROC ~85-90%）仍低于专用特征建模方法（PatchCore ~98%+），但对计算资源的需求是后者的 10 倍以上。在资源极度受限的工业边缘场景，这种"以资源换上限"的路线缺乏实用性。

4. **替代方案的可行性**：文献中的 EfficientAD（2024）等轻量蒸馏方案提供了后续跟进的路径：通过知识蒸馏将大模型能力迁移至小网络。这在本项目的硬件上可行，但需要在论文发表后独立实现蒸馏管线，作为后续扩展方向更合理。

四类路线中三类已完成复现与评测，第四类（大模型）暂不实现有明确的技术理由，后续可跟进 EfficientAD 蒸馏路线。
...
选型只遵循一条原则：**让不同的建模思路在相同条件下"对话"**。四类方法代表了对"正常模式"的四种不同定义，只有思路不同，对比才有意义，才能看出工业场景下哪种定义最有效。
...
PatchCore 将异常检测定义为一个**最近邻检索问题**：收集所有正常样本的局部特征构建"正常记忆库"，测试时将待测图像的特征在库中查找最近邻，距离最近邻越远则越异常。思路朴素但有效，它不对正常模式的分布做任何参数假设，用最原始的距离度量完成检测。

#### 3.2.2 网络架构

<figure>
  <img src="images/report/patchcore_arch.png" alt="PatchCore 网络架构" style="display:block;margin:12px auto;max-width:100%;width:620px;">
  <figcaption align="center">图 3 PatchCore 网络架构</figcaption>
</figure>
...
<figure>
  <img src="images/report/padim_arch.png" alt="PaDiM 网络架构" style="display:block;margin:12px auto;max-width:100%;width:620px;">
  <figcaption align="center">图 4 PaDiM 网络架构</figcaption>
</figure>
...
**与传统自编码器的区别**：传统 AE 在像素空间做重构，但正常和异常图像在像素值上差异本就细微。FRE 先在特征空间做重构，ResNet 将图像映射到语义特征空间后，正常和异常的差异被放大，所以 FRE 的像素级定位明显优于传统 AE。

#### 3.4.2 网络架构

<figure>
  <img src="images/report/fre_arch.png" alt="FRE 网络架构" style="display:block;margin:12px auto;max-width:100%;width:620px;">
  <figcaption align="center">图 5 FRE 网络架构</figcaption>
</figure>
...
<figure>
  <img src="images/report/draem_arch.png" alt="DRAEM 网络架构" style="display:block;margin:12px auto;max-width:100%;width:620px;">
  <figcaption align="center">图 6 DRAEM 网络架构</figcaption>
</figure>
...
选取理由：bottle 代表简单背景的物体检测（纯色背景，目标居中），carpet 代表复杂纹理检测（规律性花纹），两者特性互补，能覆盖两类典型场景。
...
所有数据集统一进行以下预处理：
- 图像尺寸统一为 256×256（letterbox 缩放保持长宽比）
- 灰度图复制为三通道
...
> 该配置（游戏本）接近边缘产线可用的典型算力水平。所有推理时间和显存测量均在此硬件上完成，数据可直接参考。
...
<figure>
  <img src="images/report/experiment_pipeline.png" alt="实验实施流程" style="display:block;margin:12px auto;max-width:100%;width:900px;">
  <figcaption align="center">图 7 实验实施流程</figcaption>
</figure>
...
四项指标的基准对比以热力图呈现（深色=高分，最优列描紫边；PRO 单独用暖色突出"瓶颈"），精确数值见附录 A：
...
<figure>
  ...
  <figcaption align="center">图 8 基准对比热力图（AUROC / AUPR / Pixel AUROC / PRO，4 算法 × 6 数据集）</figcaption>
</figure>

> PatchCore 完成了全部 6 个数据集的评测。在企业复杂场景（region2/region5）上，它的图像级检测精度与 PaDiM 互有胜负，但像素级定位（Pixel AUROC/PRO）领先。热力图上能看出规律：bottle/carpet 行整体深色（公开数据容易），region2 行整体浅色（所有方法都低），FRE 列在企业数据上明显偏浅（对域偏移敏感）。

**关键发现**：

1. **PatchCore 在图像级与像素级均领先**。bottle 上达到 100%（即所有异常样本的分数都高于所有正常样本，ROC 曲线完全分离），carpet 98.7%，region1 99.8%，region2 78.6%（该数据集最优）。在全部 6 个数据集上，PatchCore 以 97.94% 平均 Pixel AUROC 和 51.06% 平均 PRO 领先所有方法，说明记忆检索范式在工业像素级定位上有优势。
...
3. **FRE 性能分化严重**。在 bottle（99.4%）和 carpet（94.2%）上表现尚可，但在企业数据上大幅下降（region1 75.6%，region5 57.4%）。线性自编码器对域偏移极其敏感：在 ImageNet 特征空间中，触控面板的灰度图特征分布与训练集见过的自然图像差异过大，线性映射无法有效捕捉。

4. **DRAEM 在企业复杂场景下有独特优势**。region3（98.8%）和 region5（90.8%）上 DRAEM 取得最优，且其端到端像素级输出提供了天然的分割结果。自监督判别范式在高质量数据（MVTec AD）上不一定优于特征建模，但在低质量、高噪声的真实工业数据上泛化性更好。

5. **region2 是所有方法的"滑铁卢"**。PatchCore 以 78.57% AUROC 取得该数据集最优，但较其他数据集仍有巨大差距。四位算法在 region2 上的排名与整体排名一致（PatchCore > PaDiM > DRAEM > FRE），说明 region2 的难点是数据层面的（噪声大、缺陷与正常样本差异小），而非算法层面的。另外，PatchCore **在 4/6 数据集上取得最优 AUROC**（bottle、carpet、region1、region2）。

**AUPR 暴露的误报问题**：AUPR 在企业数据（缺陷率 5-25%）上的急剧下降说明一个事实：**AUROC > 90% 不意味着模型在实际使用中表现好**。region2 上 PaDiM 的 AUROC 为 73.5%、AUPR 仅 46.9%，意味着模型在保持合理检出率的同时会产生大量误报。

#### 5.1.2 像素级定位性能

像素级 Pixel AUROC 与 PRO 的精确数值见附录 A 及上方热力图（图 8 右两列）。PRO 列的暖色（YlOrRd）集中呈现了所有算法在 region2/region5 上的低谷。

**像素级定位的两个核心发现**：

1. **Pixel AUROC 和 PRO 之间存在巨大鸿沟**。以 PatchCore @ bottle 为例，Pixel AUROC 98.6% 但 PRO 仅 80.1%，相差 18.5 个百分点。这正是 PRO"平等对待"特性的体现：模型对大缺陷定位好（Pixel AUROC 高），但小缺陷和缺陷边缘覆盖不足（PRO 低）。

2. **PRO 在企业数据上普遍低迷**（多数 < 50%，region2 仅 10-16%），这是所有算法共同面临的瓶颈。但 PatchCore 在 region3 上取得了 69.51% 的企业数据最高 PRO，可见特征检索在像素级定位上有潜力。

3. **PatchCore 在 region2 的像素级优势**：在 region2 上 PatchCore 的 Pixel AUROC（92.87%）和 PRO（15.93%）均领先 PaDiM（73.87% / 10.19%），这与两个方法在 region1 上的趋势一致：特征检索在像素级的区分度优于概率建模。但 region2 极低的 PRO（15.93%）表明该区域缺陷标注本身存在较大不确定性。

### 5.2 参数消融实验

消融实验在 bottle 数据集上进行，针对每种算法的关键参数进行受控网格搜索，以敏感性折线图呈现参数对指标的影响（精确数值见附录 B）。

#### 5.2.1 PatchCore: coreset_sampling_ratio

<figure>
  <img src="images/report/ablation_patchcore.png" alt="PatchCore coreset_sampling_ratio 敏感性" style="display:block;margin:12px auto;max-width:100%;width:680px;">
  <figcaption align="center">图 9 PatchCore coreset_sampling_ratio 敏感性曲线</figcaption>
</figure>

**分析**：Image AUROC 全程保持 100%（折线水平），说明即使记忆库压缩 100 倍（coreset=0.01），检索质量依然足够区分正常/异常。PRO 随采样率增加而提升（0.01→0.20 提升 2.2 个百分点），但边际收益递减：0.10→0.20 仅提升 0.17%。默认值 0.10 在精度与效率之间取得了较好的平衡。

#### 5.2.2 PaDiM: backbone

<figure>
  <img src="images/report/ablation_padim.png" alt="PaDiM backbone 消融" style="display:block;margin:12px auto;max-width:100%;width:680px;">
  <figcaption align="center">图 10 PaDiM backbone 消融（2 点连线）</figcaption>
</figure>

**分析**：WideResNet-50-2 相比 ResNet-18 在 Pixel AUROC 上提升 0.23%，在 PRO 上大幅提升 5.97%。更强大的 backbone 提供更丰富的特征表示 → 高斯模型能更精确地刻画正常 patch 的分布边界 → 马氏距离对微小异常的敏感度提高。代价是参数量从 2.8M 增加到约 69M。**服务器场景建议 wide_resnet50_2；边缘端保留 resnet18**。

#### 5.2.3 FRE: latent_dim

<figure>
  <img src="images/report/ablation_fre.png" alt="FRE latent_dim 敏感性" style="display:block;margin:12px auto;max-width:100%;width:680px;">
  <figcaption align="center">图 11 FRE latent_dim 敏感性曲线</figcaption>
</figure>

**分析**：Image AUROC 在 latent_dim=220 时达到峰值 99.52%，但 PRO 随 latent_dim 增加而单调下降。这是典型的信息瓶颈（Information Bottleneck）效应：latent_dim 越小，AE 被迫学习更紧凑的表示，对正常模式的刻画更精细；latent_dim 越大，AE "容错"能力越强，对细微异常的区分度下降。默认值 220 在图像级分类精度上最优；若以定位精度优先，应降至 100。
...
基于 Youden's J 最优阈值，对每个模型-数据集组合计算混淆矩阵（数据源：`results/confusion_matrices/*.json`）。以下为代表性结果：

| 组合 | TP | FP | TN | FN | 准确率 | 精确率 | 召回率 |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| PatchCore @ bottle | 63 | 0 | 20 | 0 | **100%** | **100%** | **100%** |
| PaDiM @ bottle | 62 | 0 | 20 | 1 | 98.8% | 100% | 98.4% |
| FRE @ bottle | 55 | 0 | 20 | 8 | 90.4% | 100% | 87.3% |
| DRAEM @ bottle | 60 | 0 | 20 | 3 | 96.4% | 100% | 95.2% |
| PatchCore @ region1 | 7 | 8 | 83 | 0 | 91.8% | 46.7% | 100% |
| PaDiM @ region2 | 4 | 0 | 91 | 11 | 89.6% | 100% | 26.7% |

**混淆矩阵揭示了 AUROC 无法展示的信息**：

- **DRAEM @ bottle**：AUROC 97.7%，FP=0（无正常图误判），但在最优阈值下漏检 3 张缺陷（FN=3，召回 95.2%）。说明 AUROC 高 ≠ 召回无损耗，缺陷样本的分数分布仍可能与阈值临界值重叠。
- **PaDiM @ region2**：AUROC 73.5%，FP=0（无误报），但 FN=11（15 张缺陷中漏检 11 张，召回仅 26.7%）。阈值 0.54 偏保守，漏检了大量缺陷。产线上若直接采用此阈值会有严重的漏检风险，必须降低阈值、牺牲精确率来换取可用的召回率。这与 region2 高噪声、缺陷与正常样本分数分布重叠直接相关。
- **PatchCore @ bottle**：TP=63, FP=0, TN=20, FN=0。完美分离，与 AUROC 100% 一致，是"零误报零漏检"的理想状态。
...
<figure>
  <img src="images/report/small_sample_dual_axis.png" alt="小样本鲁棒性双轴折线 + 评分卡" style="display:block;margin:12px auto;max-width:100%;width:900px;">
  <figcaption align="center">图 12 小样本鲁棒性分析（上：AUROC 实线/PRO 虚线双轴；下：鲁棒性评分卡）</figcaption>
</figure>

上图同时呈现图像级 AUROC（左轴实线）与像素级 PRO（右轴虚线）随样本量的变化，DRAEM PRO 暴跌段以浅红阴影标注"定位退化"。下方评分卡汇总鲁棒性分数（= N30 AUROC / N150 AUROC），PatchCore 行高亮。完整分 N 数值见附录 C。

> ¹ DRAEM 在 N=150 时 AUROC 反而略低于 N=30，可能原因是：DRAEM 的异常合成过程引入随机性，在小样本下合成策略的多样性补偿了样本量的不足。需更大的重复实验验证。

**核心结论**：

1. **PatchCore 对小样本完全不敏感**：N=30 时 AUROC 已经是 100%，PRO 仅比 N=150 时低 1.6%。原因在于它"不学习"：记忆库的大小和质量随样本量变化，但检索机制本身不受影响。只要 30 张正常样本能覆盖足够的特征多样性，检索就能正常工作。
...
<figure>
  <img src="images/report/radar_chart.png" alt="四种算法多维能力对比雷达图" style="display:block;margin:12px auto;max-width:100%;width:720px;">
  <figcaption align="center">图 13 四种算法多维能力对比雷达图</figcaption>
</figure>
...
> **排名依据说明**：PaDiM 在平均图像级 AUROC（92.44%）与平均 PRO（52.17%）上略高于 PatchCore，但 PatchCore 综合居首的依据是**像素级定位精度（Pixel AUROC 97.94%，领先 PaDiM 3.0 个百分点）**：像素级定位是工业缺陷检测的主要交付能力，且 PatchCore 在 4/6 数据集上取得最优 AUROC。PaDiM 的优势在于轻量（2.8M 参数）与边缘部署友好，详见第六章分场景推荐。
...
本课题基于 **FastAPI + Alpine.js** 构建了一套交互式 Web 可视化验证平台（Phase 2），满足课题验收的可视化要求。平台支持算法介绍、模型推理、训练工作室与四模型对比，采用 CSS scroll-snap 全屏吸附的四页架构。早期的 Gradio 版本作为 legacy fallback 保留。

**访问方式**：本地启动后浏览器访问 `http://127.0.0.1:8000`（`python scripts/run_ui.py`）。Legacy Gradio：`python scripts/run_ui.py --gradio`（`http://127.0.0.1:7860`）。

### 7.2 核心功能

平台采用 CSS scroll-snap 全屏吸附的四页架构，每页 100dvh 全视口，滚动吸附到整页：

| 页面 | 功能模块 | 说明 |
|------|----------|------|
| s0 算法介绍 | 四算法卡片 + 流程图轮播 | 展示四算法技术路线与架构图 |
| s1 训练工作室 | 样本上传 + SSE 训练流 | 上传正常样本、配置超参数、实时 loss 曲线监控 |
| s2 单模型推理 | 算法/来源/数据集选择 → SSE 推理 | 选定算法推理，输出异常热力图 + NMS 边界框 + 异常分数 |
| s3 四模型对比 | 同一测试图 × 4 算法并行推理 | SSE 流式并排展示，每槽位含热力图与得分 |

平台还支持结果可视化（原图/热力图对比滑块、异常得分 tooltip）、预训练与自训练两种模型来源切换、亮/暗双模式。

### 7.3 技术架构

<figure>
  <img src="images/report/architecture.png" alt="可视化验证平台技术架构" style="display:block;margin:12px auto;max-width:100%;width:900px;">
  <figcaption align="center">图 14 可视化验证平台技术架构</figcaption>
</figure>
...
| 层级 | 技术栈 | 核心职责 |
|------|--------|----------|
| **前端层** | Alpine.js SPA + CSS scroll-snap | 四页吸附导航、主题切换、SSE 流消费与渲染 |
| **业务逻辑层** | FastAPI + anomalib 2.3 | `/api/predict`、`/api/compare`、`/api/train` 等 SSE 端点；`AnomalyDetector` 类完成模型加载 → `Engine.predict()` 推理 → 后处理（热力图叠加 + NMS 边界框）；推理与训练通过 `asyncio.to_thread` 在线程池执行 |
| **数据与模型层** | 文件系统 + GPU 显存 | 4 种算法 checkpoint 的自动匹配与加载（预训练/自训练两种来源） |

**推理数据流**：原始图像 → 预处理 (256×256) → 模型前向传播 → 异常分数图 → NMS 边界框 → SSE 流推送前端渲染

### 7.4 说明

该平台满足课题验收的可视化要求，但企业实际部署将采用集成度更高的后端 API 方案，不直接使用该前端。
...
本课题在 ≤150 张正常样本的约束下，对无监督工业异常检测领域的三类技术路线、四种代表性算法进行了复现与评测。在 MVTec AD 公开数据集和征图 Focusight 企业真实数据集上，共完成 24 组基准对比实验（4 算法 × 6 数据集全覆盖）、9 组消融实验、16 组小样本鲁棒性实验和 24 组混淆矩阵分析。主要结论如下：

1. **特征建模类方法（PatchCore/PaDiM）是当前工业场景下综合表现最优的技术路线**。两类方法均无需梯度训练，小样本鲁棒性极强（N=30 时 image_AUROC 保持 ≥ 99.4%），部署灵活：PatchCore 适合服务器端高精度检测，PaDiM（2.8M 参数）适合边缘端轻量部署。
...
3. **特征重构类方法（FRE）在域偏移下鲁棒性不足**。在 MVTec AD 上表现尚可（bottle 99.4%），但在企业灰度图数据上大幅退化（region5 仅 57.4%）。ImageNet 预训练特征与工业灰度图之间的域差异，对重构范式是根本性挑战。

4. **小样本鲁棒性实验反映出不同范式的本质差异**：不需要训练的方法（PatchCore/PaDiM）几乎不受样本量影响；需要训练的方法（FRE/DRAEM）在像素级定位上受样本量影响显著，其中 DRAEM 的 PRO 在 N=30 时较 N=150 时下降约 38%。

5. **PRO 指标是当前无监督异常定位的共性瓶颈**。所有方法在所有数据集上的 PRO 均显著低于其他三项指标，企业数据上多数 < 50%。像素级定位的连续性（区域完整性）仍是未解决的问题。
...
1. **PRO 专项优化**：引入 CRF（条件随机场）后处理、多尺度融合、自适应阈值策略，全面提升定位连续性。目标：企业数据 PRO 提升至 60%+。
...
> 数据集：bottle，样本量从 209 张训练集中随机采样，seed=42。各 N 的分项数值已在 5.5 节双轴折线图中呈现，此处保留汇总表供查阅。

### C.1 小样本鲁棒性汇总 (Image AUROC %)
